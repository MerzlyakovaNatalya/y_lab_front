import React, { useState, ChangeEvent, useCallback, memo, useEffect } from 'react'
...
const Textarea = React.forwardRef<HTMLTextAreaElement, IProps>(
  ({ placeholder, value, isBold, addMessage, adjustTextareaHeight }, ref) => {
    ...
    return (
      <>
        {showPicker && (
          <Picker lazyLoadEmojis={true} onEmojiClick={onEmojiClick} autoFocusSearch={true} />
        )}
        <img className={cn('emoji-icon')} src={Img} onClick={() => setShowPicker(val => !val)} />
        <textarea

...
function CatalogFilterForModal() {
  ...
  const callbacks = {
    // Сортировка
    onSort: useCallback(
      (sort: string | number) => store.actions.catalog_modal.setParams({ sort }),
      [store],
    ),
    // Поиск
    onSearch: useCallback(
      (query: string) => store.actions.catalog_modal.setParams({ query, page: 1 }),
      [store],
    ),
    // Сброс
    ...
    onCategory: useCallback(
      (category: string | number) => store.actions.catalog_modal.setParams({ category, page: 1 }),
      [store],
    ),
  }

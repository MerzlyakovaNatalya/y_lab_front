...
export default function useSelector<State, Selected>(
  selectorFunc: SelectorFunc<State, Selected>,
): Selected {
  const store = useStore()

  const [state, setState] = useState(() => selectorFunc(store.getState() as State))
